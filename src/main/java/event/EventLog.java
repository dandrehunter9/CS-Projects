...
//************************************************************
//
//	$Source: /cvsroot/ATM/implementation/event/event.EventLog.java,v $
//
 ...
 * We first have the date and time of the event, followed by an indication of the class in which the event.Event was generated,
 ...
 * this string
 *
 * This class implements the SINGLETON pattern. Hence, there will only be one instance of event.EventLog
 */
//==============================================================
public class EventLog
{
    // data members
    /**
     * The single event.EventLog instance
     */
	private static EventLog instance;
	
	/**
	 * The name of the file holding the event.Event Log
	 */
	...
	/** Create and return an instance of event.EventLog. This method
	 *  enforces a Singleton pattern. 
	 *
	 * @return	The single event.EventLog instance
	 */
	//----------------------------------------------------------
	...
	protected EventLog ()
	{
		...
		if(logFile.length() > MAX_SIZE)
		{
			...
			{
				new Event(Event.getLeafLevelClassName(this), "event.EventLog", "Could not backup log file ", Event.WARNING);
			}
		}
	}
	 ...
	 *
	 * @param	event	The event.Event object to record in the log
	 */
	//-------------------------------------------------------------
		...
		while (!done)
		{
			...
			else
			{
				...
				if (posOfClosePlaceHolder < 0)
				{
					...
					if (displaySubErrors)
						new Event("event.EventLog", "String replaceParameters", "No close placeholder corresponding to open placeholder in source string "+source, Event.WARNING);
				...
				else
				{
					...
					try
					{
					...
					catch (NumberFormatException ex )
						{
							...
							if (displaySubErrors)
								new Event("event.EventLog", "String replaceParameters", "Specified parameter index "+paramIndexStr+" not an integer ", Event.WARNING);
							paramString.append(source.substring(currentStringPosition, strlen));				
						}
					try
					{
					...
					// If not, we display a parsing sub-error and append the rest of the description string - to return later
					catch (ArrayIndexOutOfBoundsException ex)
						{
							...
							if (displaySubErrors)
								new Event("event.EventLog", "String replaceParameters", "No parameter provided to correspond to index "+paramIndex, Event.WARNING);
							paramString.append(source.substring(currentStringPosition, strlen));				
						}
					if (!done)
					{
