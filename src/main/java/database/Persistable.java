...
//			DEBUG: System.err.println( "An SQL Error Occured:" + sqle + "\n" + sqle.getErrorCode() + "\n" + sqle.getMessage() + "\n" + sqle);
					...
					Properties where)
    {
		...
		try
		{
			...
			//new event.Event(event.Event.getLeafLevelClassName(this), "getPersistentState", "Attempting to get database connection", event.Event.DEBUG);

			theDBConnection = myBroker.getConnection();

			//new event.Event(event.Event.getLeafLevelClassName(this), "getPersistentState", "Return from database connection attempt call",
			//	event.Event.DEBUG);
...
//	Put in debug messages via event.event.Event
//
...
//	Changed event.Event method invocations to lower case.
//
